...
export const ChatView: React.FC<ChatViewProps> = ({ chatSession, profile, initialMessages = [], onReset, onRegenerateAvatar, onGoHome }) => {
  ...
  return (
    <div className={`flex flex-col h-full w-full relative transition-colors duration-500 ease-in-out ${theme.containerBg} ${theme.fontFamily}`}>
      ...
      {/* Header */}
      <div className={`flex-none flex items-center px-5 py-4 backdrop-blur-xl border-b z-20 ${theme.headerBg} ${theme.menuBorder || 'border-white/5'}`}>
        
        {/* Left Side: Menu Button */}
        <div className="relative mr-4" ref={menuRef}>
           <button onClick={() => { triggerHaptic(10); setIsMenuOpen(!isMenuOpen); }} className="p-2 -ml-2 text-gray-300 hover:text-white">
               <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg>
           </button>
           {isMenuOpen && (
            <div className={`absolute left-0 top-full mt-2 w-64 rounded-2xl shadow-2xl border backdrop-blur-2xl animate-fade-in z-50 ${theme.menuBg} ${theme.menuBorder}`}>
              <div className="py-1">
        ...
        {/* Name and Status (Avatar removed) */}
        <div className="flex-1">
            <h3 className={`text-base font-bold leading-none mb-1 ${theme.headerText}`}>{profile.name}</h3>
            <div className="flex items-center space-x-2">
                <span className={`w-1.5 h-1.5 rounded-full ${moodStyle.color} animate-pulse`}></span>
