...
export const generateRandomProfileData = async (genderPreference: 'male' | 'female' | 'any'): Promise<Partial<CharacterProfile>> => {
    ...
    Output ONLY valid JSON with the following structure:
    {
        ...
        "intimacyLevel": "normal" | "medium" | "high" (randomly choose)
    }
...
export const createChatSession = (profile: CharacterProfile, historyMessages: Message[] = []): Chat => {
  ...
  let mode = 'NORMAL/TEASING/FRIENDLY';
  if (profile.intimacyLevel === 'medium') mode = 'ROMANTIC/FLIRTY/TEASING';
  if (profile.intimacyLevel === 'high') mode = 'BOLD/INTENSE/EXPLICIT/UNFILTERED';
