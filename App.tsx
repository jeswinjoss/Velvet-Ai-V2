...
import { AppState, CharacterProfile, Message, ThemeId } from './types';
...
const App: React.FC = () => {
  ...
  const [loadingThemeId, setLoadingThemeId] = useState<ThemeId>('midnight');
  ...
  useEffect(() => {
    ...
    // Always go to HOME to show the UI and Ribbon
  ...
  // Background Logic with Intimacy Level Overrides (Cooler Tones)
  useEffect(() => {
    const determineColors = () => {
        // Check for Intimacy Level Overrides from Active Profile
        const currentProfile = profiles.find(p => p.id === activeProfileId);
        if (currentProfile && appState !== AppState.SETUP) {
            if (currentProfile.intimacyLevel === 'normal') {
                // Fresh Mint/Sky
                setBgColors({ blob1: 'bg-sky-500/20', blob2: 'bg-teal-400/15', blob3: 'bg-emerald-500/10' });
                return;
            }
            if (currentProfile.intimacyLevel === 'high') {
                // Electric Violet (was Pink)
                setBgColors({ blob1: 'bg-violet-600/20', blob2: 'bg-fuchsia-500/15', blob3: 'bg-purple-600/10' });
                return;
            }
            // Medium falls through to standard time-of-day logic below
        }

        // Standard Time-of-Day Logic (Cool aesthetic)
        ...
        if (timePeriod === 'morning') {
            // Cool Dawn: Sky + Indigo
            setBgColors({ blob1: 'bg-sky-400/20', blob2: 'bg-indigo-300/15', blob3: 'bg-blue-300/10' });
        } else if (timePeriod === 'afternoon') {
            // Bright Day: Cyan + Blue
            setBgColors({ blob1: 'bg-cyan-500/15', blob2: 'bg-blue-500/15', blob3: 'bg-sky-400/10' });
        } else {
            // Deep Night: Slate + Violet
            setBgColors({ blob1: 'bg-brand-900/20', blob2: 'bg-violet-900/15', blob3: 'bg-slate-800/20' });
        }
    };
  ...
  }, [manualTimeOfDay, activeProfileId, appState, profiles]);
  ...
  const handleSetupComplete = async (newProfile: CharacterProfile) => {
    setLoadingThemeId(newProfile.themeId);
  ...
  const handleRegenerateAvatar = async () => {
    ...
    setLoadingThemeId(profile.themeId);
    setIsGenerating(true);
    try {
        usageService.trackRequest();
  ...
  const handleRandomReject = () => {
      setRandomCandidate(null);
      handleRandomCompanion(lastGenderPref);
  };
  ...
  // Check for High Intimacy to show Love Elements (Cooler Colors)
  const showLoveElements = activeProfileId && 
                           profiles.find(p => p.id === activeProfileId)?.intimacyLevel === 'high' && 
                           appState !== AppState.SETUP;
  
  const loadingTheme = THEMES[loadingThemeId] || THEMES.midnight;

  return (
    <div className="h-[100dvh] w-full bg-[#020617] text-white flex items-center justify-center relative overflow-hidden selection:bg-brand-500/30">
        ...
        {/* Faded Love Elements (Cooler Palette for Fresh Look) */}
        {showLoveElements && (
            <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 opacity-30">
                <div className="absolute top-[10%] left-[20%] text-violet-500 animate-[float_6s_ease-in-out_infinite] opacity-50 text-4xl">♥</div>
                <div className="absolute top-[40%] right-[15%] text-fuchsia-400 animate-[float_8s_ease-in-out_infinite_reverse] opacity-40 text-2xl">♥</div>
                <div className="absolute bottom-[20%] left-[10%] text-indigo-600 animate-[float_7s_ease-in-out_infinite] opacity-30 text-6xl">♥</div>
                <div className="absolute top-[60%] left-[50%] text-purple-300 animate-[float_9s_ease-in-out_infinite_reverse] opacity-20 text-3xl">♥</div>
            </div>
        )}

        {/* Main Container */}
        <div className="relative z-10 w-full h-full md:h-[90vh] md:max-h-[850px] md:max-w-md lg:max-w-lg flex flex-col shadow-2xl md:rounded-3xl overflow-hidden bg-dark-900/60 backdrop-blur-md border-white/5 md:border">
            ...
            {appState === AppState.GENERATING_PROFILE && !isGenerating && (
                <LoadingScreen message="Finalizing..." theme={loadingTheme} />
            )}
            ...
            {appState === AppState.RANDOM_REVIEW && randomCandidate && (
                <div className="absolute inset-0 z-30 bg-dark-900/95 flex flex-col animate-fade-in">
                     <button 
                     ...
                     <div className="absolute bottom-8 left-0 w-full px-8 flex items-center justify-between">
                         ...
                         <button 
                            ...
                            className="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-500/50 text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all"
                         >
            ...
            {appState === AppState.UPDATE_CHECK && (
                <UpdateOverlay onComplete={(found) => {
                    ...
                    else showToast("App is latest version", "info");
                }} />
            )}

            {/* Global Loading Overlay */}
            {isGenerating && (
                <div className="absolute inset-0 z-50 bg-dark-900/60 backdrop-blur-md flex flex-col justify-center animate-fade-in">
                    <LoadingScreen 
                        message={appState === AppState.SETUP ? "Updating companion..." : "Dreaming up someone special..."} 
                        theme={loadingTheme}
                    />
                </div>
            )}
